...
*/
void race_condition(int i);

/**
good function, uses mutex to fix race condition
@param i an int to print 1000 times
*/
void fixed_race_condition(int i);
...
int main() {
  ...
  // display results
	...
	/* SECOND PART OF DEMO: RACE CONDITIONS */

	std::cout << "\nWith races:\n";

	std::thread race1(race_condition, 1);
	std::thread race2(race_condition, 2);
	...
	std::thread no_race1(fixed_race_condition, 1);
	std::thread no_race2(fixed_race_condition, 2);
